#!/usr/bin/env python3
"""
Test multi-turn conversation with streaming enabled
"""
...
def process_turn_with_streaming(messages, user_input, turn_num):
    """Process a single turn with streaming enabled"""
    ...
    try:
        ...
        if response.choices[0].message.tool_calls:
            ...
            print(f"\nGetting final response with STREAMING...")
            ...
            for chunk in stream_response:
                ...
                if chunk_count > 100:  # Safety limit per turn
                    print("\n[Truncated - too many chunks]")
                    break
            
            print(f"\n[Streaming completed: {len(collected_content)} chars in {chunk_count} chunks]")
            ...
            # Handle regular response (no tool calls) - also with streaming
            print("Regular response - using streaming...")
            stream_response = client.chat.completions.create(
                model=MODEL, 
                messages=messages,
                stream=True
...
def test_multiturn_streaming():
    """Test multi-turn conversation with streaming enabled"""
    print("Testing Multi-Turn Streaming Conversation")
    print("=" * 50)
    ...
    for turn_num, user_input in enumerate(conversation_turns, 1):
        success = process_turn_with_streaming(messages, user_input, turn_num)
        if not success:
            return False
    
    print(f"\n{'='*50}")
    print(f"Multi-turn streaming conversation completed successfully!")
    print(f"Final conversation length: {len(messages)} messages")
    
    return True

if __name__ == "__main__":
    success = test_multiturn_streaming()
    
    if success:
        print("\n🎉 MULTI-TURN STREAMING TEST PASSED!")
        ...
        print("\n✅ All streaming functionality is now working!")
    else:
        print("\n❌ MULTI-TURN STREAMING TEST FAILED!")
